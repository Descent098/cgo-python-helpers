...
export class Struct {
    ...
    */
    constructor(name, attributes){
        this.structID = crypto.randomUUID();
...
export function addStruct(){
    // Create struct data
    const structInstance = new Struct("MyStruct", [new Attribute("myAttribute", AttributeTypes.str)]);

    // Add struct to global mapping
    currentStructs.set(structInstance.structID, structInstance);

    renderStructs() // Re-render structs
 ...
 */
export function removeStruct(structID){
 ...
 */
export function updateStructName(structID){
 ...
 */
export function updateStructAttributes(fieldSetElement, structID){
...
export function renderStructs(){
    const structElement = document.getElementById("structs");
    // Generate new HTML
    let newHTML = "";
    for (const [id, struct] of currentStructs.entries()){
        let structLabel = (struct.name !== "MyStruct") ? struct.name : `MyStruct-${struct.structID}`;
